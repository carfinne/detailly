...
interface SzenenProps {
  ...
  /** Pin-Beschriftungen (i18n) — von der Seite aufgelöst und durchgereicht,
   *  damit useT() NICHT über die react-three-fiber-Canvas-Grenze muss. */
  pinLabels?: string[];
}

function HoloSzene({ containerRef, branche, reduziert, eingabe, onErsterFrame, onPings, pinLabels }: SzenenProps) {
...
export interface LandingCar3DProps {
  ...
  onFehler?: () => void;
  /** Pin-Beschriftungen (i18n), Reihenfolge = Scan-Reihenfolge. Ohne Angabe
   *  greifen die deutschen Defaults der Szene. */
  pinLabels?: string[];
  /** Barrierefreier Name des Showcase-Containers (i18n). */
  ariaLabel?: string;
}

export default function LandingCar3D({ branche, fallback, onReady, onPings, onFehler, pinLabels, ariaLabel }: LandingCar3DProps) {
  ...
  return (
    <div
      ref={containerRef}
      role="img"
      aria-label={ariaLabel ?? 'Interaktives 3D-Fahrzeugmodell mit markierten Schadenspunkten'}
