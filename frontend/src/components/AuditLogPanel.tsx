...
import { Loading, ErrorBox, UpgradeHinweis, Empty, SectionCard } from '@/components/ui';
...
/**
 * Audit-Log als Einstellungen-Panel (Tab „Audit-Log"). Fetching/Logik sind
 * unveraendert aus der frueheren /audit-Seite uebernommen; nur die Praesentation
 * ist an die Einstellungen-Tabs angepasst (SectionCard statt eigenem PageHeader).
 */
export function AuditLogPanel() {
  ...
  return (
    <div className="space-y-5">
      ...
      {!error && (
        <SectionCard title={t('audit.title')} subtitle={t('audit.subtitle')}>
          {loading ? (
            <Loading />
          ) : items.length === 0 ? (
