...
/**
 * 1-Klick-Abmeldung (§ 7 UWG). Liest das Token clientseitig aus der URL und ruft
 * die oeffentliche API – sofort wirksam. Statisch exportierbar (kein
 * useSearchParams/Server-Query-Zugriff).
 */
export default function NewsletterAbmeldenPage() {
  ...
  useEffect(() => {
    ...
    let aktiv = true;
    api
      .post('/public/newsletter/abmelden', { token })
  ...
  return (
    <PublicShell>
      <PublicBrandHeader
        backHref="/"
        small
        title={t('newsletter.unsub.title')}
        subtitle={t('newsletter.unsub.subtitle')}
      />
      ...
      {status === 'loading' ? (
        ...
        <div className="card space-y-4 text-center">
          ...
          <h2 className="font-display text-lg font-semibold text-chrome-50">{t('newsletter.unsub.successTitle')}</h2>
          <p className="text-sm text-chrome-300">{t('newsletter.unsub.success')}</p>
        ...
        <div className="card space-y-4 text-center">
          <div className="flex items-center gap-2 rounded-xl border border-danger/30 bg-danger-soft px-3 py-2.5 text-left text-sm text-danger">
            ...
            {status === 'missing' ? t('newsletter.unsub.missing') : t('newsletter.unsub.error')}
          </div>
          <Link href="/" className="btn-subtle w-full">{t('newsletter.backHome')}</Link>
        </div>
      )}
